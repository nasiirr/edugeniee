...
with st.expander("ℹ️ Setup Information"):
    ...
    **Gemini API:**
    - Set `GEMINI_API_KEY` in `.env` file or Streamlit secrets
    
    **Firebase/Firestore:**
    - Set `FIREBASE_CREDENTIALS_PATH` (path to service account JSON)
    - OR set `FIREBASE_CREDENTIALS_JSON` (raw JSON string)
    - OR configure `[firebase_credentials]` in Streamlit secrets
    
    ### 📖 How to Use
    1. Enter your study topic
    2. Select audience and goal
    3. Click "Generate Content"
    4. Review the generated material
    5. Click "Save to Firestore" to save
    6. Access saved materials anytime with "Access Saved Content"
    """)
